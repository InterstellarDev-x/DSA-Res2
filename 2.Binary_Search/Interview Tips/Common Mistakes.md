...
| 4 | Off-by-one: `hi = n-1` for lower bound | `int hi = n - 1` | `int hi = n` — answer may be at index n ("not found") |
| 5 | Accessing `nums[mid-1]` without bounds check | `if (nums[mid] == nums[mid-1])` | Guard: `mid > 0 &&` |
| 6 | Wrong feasibility `lo`/`hi` | `lo = 0, hi = max` for capacity | `lo = max(weights)` — must carry single heaviest item |
| 7 | Not verifying target exists after lower bound | `return lowerBound(nums, target)` for exact search | Check `arr[lb] == target` before returning |
