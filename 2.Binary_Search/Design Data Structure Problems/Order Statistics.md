...
|----------|-----|-------------|-------|-------|
| Sorted ArrayList + binary search | O(n) | O(log n) | O(n) | Simple, slow add |
...
public class OrderStatisticsBIT {
    ...
    private final int[] bit = new int[MAX + 1];
...
public int kthSmallestFast(int k) {
    int pos = 0;
    for (int pw = Integer.highestOneBit(MAX); pw > 0; pw >>= 1) {
...
- k > total elements added → throw `NoSuchElementException`
...
3. **How does Java's `TreeMap` handle this?** → `TreeMap.headMap(val).size()` for rank, but O(n) for kth unless augmented
