...
public TreeNode insertIntoBST(TreeNode root, int val) {
    if (root == null) return new TreeNode(val);   // attach at the null leaf
...
| Case | Action |
|---|---|
| **No child** (leaf) | Return `null` to the parent. |
...
**adjacent** values in that order. Walk inorder, keep the previously-visited value in `prev`, and
minimize `cur - prev`.
...
private void inorder(TreeNode node) {
    if (node == null) return;
    inorder(node.left);
    if (prev != null) minDiff = Math.min(minDiff, node.val - prev); // sorted ⇒ node.val ≥ prev
    prev = node.val;
    inorder(node.right);
}
```
...
| Min Abs Diff (530) | inorder + track `prev` | O(n) | O(h) |
