...
// FIX: use >>> for unsigned (fill-with-0) shift
n >>> 1; // = 2147483644 (01111100)
```

---

## 3. Integer.MIN_VALUE Negation Overflow

```java
// BUG
int n = Integer.MIN_VALUE;
int pos = -n; // still Integer.MIN_VALUE! overflow

// FIX: cast to long
long pos = -(long) n; // = 2147483648L
```

---

## 4. `1 << k` vs `1L << k` for k >= 31

```java
// BUG: 1 is int; 1 << 31 = Integer.MIN_VALUE (negative!)
long mask = 1 << 31; // = -2147483648L (promoted after shift)

// FIX
long mask = 1L << 31; // = 2147483648L
```
...
```java
// BUG: 1 << 32 in Java = 1 << (32 % 32) = 1 << 0 = 1 (not 0!)
int fullMask = (1 << 32) - 1; // = 1 - 1 = 0 — WRONG for n=32
...
```java
// BUG: tmp << 1 can overflow long if tmp is near Long.MAX_VALUE/2
...
- [ ] Using `>>>` where sign extension would corrupt result?
- [ ] `(long)` cast before negating/abs if input could be `Integer.MIN_VALUE`?
- [ ] `1L <<` instead of `1 <<` for shifts ≥ 31?
