...
| Zigzag Level Order Traversal | 103 | O(n) | O(n) | Use `list::push_front` (O(1)) to avoid O(n) inserts |
...
| Construct from Preorder + Inorder | 105 | O(n) | O(n) | `unordered_map` of inorder indices avoids O(n) scans |
...
| Binary Tree Maximum Path Sum | 124 | O(n) | O(h) | Global best + clamped child gains `max(0, gain)` |
...
| Level Order Traversal II (bottom-up) | 107 | O(n) | O(n) | BFS then reverse, or `push_front` into a `list` |
...
| Delete Nodes And Return Forest | 1110 | O(n) | O(n) | `unordered_set` of to-delete; collect orphaned roots |
