...
So when we write space as **O(h)**, that's `O(log n)` in the best/balanced case and `O(n)` in the worst/skewed case. The same logic applies to an explicit `Vec<Box<TreeNode>>` stack — it holds at most one path's worth of nodes.
...
| Binary Tree Preorder Traversal | 144 | O(n) | O(h) | Explicit `Vec` stack mirrors recursion depth |
...
| Zigzag Level Order Traversal | 103 | O(n) | O(n) | Use `VecDeque::push_front` (O(1)) to avoid O(n) inserts |
...
| Binary Tree Maximum Path Sum | 124 | O(n) | O(h) | Global best + clamped child gains `0.max(gain)` |
...
| Average of Levels | 637 | O(n) | O(n) | BFS sum per level; use `i64`/`f64` to avoid overflow |
...
| Level Order Traversal II (bottom-up) | 107 | O(n) | O(n) | BFS then reverse, or `push_front` into a `VecDeque` |
| House Robber III | 337 | O(n) | O(h) | Return `(rob, skip)` tuple per node (tree DP) |
| Binary Tree Cameras | 968 | O(n) | O(h) | Greedy post-order states; check root after recursion |
| Distribute Coins in Binary Tree | 979 | O(n) | O(h) | Sum `balance.abs()` of moves bubbling up |
| Lowest Common Ancestor | 236 | O(n) | O(h) | Return self on match; split point is the LCA |
| LCA of Deepest Leaves | 1123 | O(n) | O(h) | Return (depth, lca) tuple bottom-up |
