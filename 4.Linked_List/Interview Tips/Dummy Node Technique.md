...
Without a dummy node, you write `if (head == null)` or `if (prev == null)` guards throughout. With a dummy node, the head is never the "first real node" — it's always a safe sentinel, and `dummy.next` is the answer.
...
**Without dummy:** If the head itself needs deletion, `prev` would be `null` — you'd need to special-case `if (prev == null) head = head.next`.
...
**Without dummy:** When `left = 1`, `pre` would be `null`.
...
void insertFront(Node node) {
    node.next = head.next; // never null (at worst, tail)
    node.prev = head;
    head.next.prev = node;
    head.next = node;
}
...
Node lru = tail.prev; // never null (at worst, head — but we check size)
remove(lru);
```

**Without sentinels:** `insertFront` must check `if (head.next == null) ...`; `removeLast` must check `if (tail.prev == null) ...`
...
Use `new ListNode(0)` — the value doesn't matter since `dummy` is never part of the answer. Some coders use `-1` or `Integer.MIN_VALUE` to distinguish from real values during debugging.
