...
For `vector<vector<int>>`, initialize with `-1` (e.g. `vector<vector<int>>(n, vector<int>(m, -1))`) **only when `-1` is not a valid answer**. When `0` is a legitimate result (counting, boolean), prefer `vector<vector<optional<int>>>` and test for `nullopt`, or pick a clearly out-of-range sentinel. This avoids the "0 collides with not-computed" bug — see [Common Mistakes](./Common%20Mistakes.md).
...
#include <bits/stdc++.h>
using namespace std;
vector<vector<int>> dp(m + 1, vector<int>(n + 1, 0));   // row 0 / col 0 are the empty-string base cases
```
...
## 7. Encode multi-dimensional state into a unordered_map key when the array is sparse/huge
...
| 7 | Pack `(i,j,k)` into a `long` key for unordered_map memo. |
