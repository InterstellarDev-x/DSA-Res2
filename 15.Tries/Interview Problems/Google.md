...
1. **Store the word at the Trie node.** Instead of `boolean isEnd`, store `String word` at terminal nodes. When the board DFS reaches a node whose `word != null`, we have a full match and can collect it without reconstructing the path from the board.
2. **Null the leaf after a word is found** (`node.word = null`). This deduplicates (the same word reachable via two paths is reported once) and signals the word is consumed.
...
class Solution {
    static class TrieNode {
        ...
        String word;   // full word stored at terminal node, else null
    }
...
**Problem.** Given `int[] nums`, return the maximum value of `nums[i] ^ nums[j]`.
    ...
    private int maxXorWith(TrieNode root, int num) {
        ...
        for (int i = HIGH_BIT; i >= 0; i--) {
            ...
            if (node.children[want] != null) {
                xor |= (1 << i);                // this bit of the XOR becomes 1
...
**Complexity.** Let `B = 31` be the bit width and `n = nums.length`. Insert is `O(B)` per number, search is `O(B)` per number, so total `O(n · B)` time and `O(n · B)` space for the Trie. This beats the naive `O(n^2)` pairwise comparison.
